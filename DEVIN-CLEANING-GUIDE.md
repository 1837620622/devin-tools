# Devin 与 macOS 清理参考手册

> 针对 **"Cascade 对话长就卡顿"** 和 **"绕过限速/刷新会话"** 两大核心诉求整理。
> 作者：传康Kk · GitHub: <https://github.com/1837620622/devin-tools>

## 0. 运行模式（重要）

工具默认运行在 **强制重置模式**。所有清理菜单完成后会**自动重置 Devin 设备 ID**（installation_id / machineid / telemetry.*）。

| 模式 | 行为 | 如何启用 |
|:-:|:-|:-|
| **强制重置（默认）** | 清理完立即重置 ID，重启 Devin 可能需要重登 | 直接运行脚本 |
| **保守模式** | 清理不动 ID，保留登录态 | `FORCE_RESET_ID=0 bash fix-devin-mac.sh` |

### 为什么默认强制重置？

- **绕过限速**：重置后 Devin 服务端把你当成新设备，免费额度重新计算
...
| `~/Library/Application Support/Devin/machineid` | 整个 UUID | 重生成 |
...
# Windows PowerShell
$env:FORCE_RESET_ID="0"; .\fix-devin-win.ps1
```
...
| 序号 | 根因 | 表现 | 对应治理 |
|:-:|:-|:-|:-|
| 1 | **对话历史文件累计过大** | 切换对话、启动慢 | `fix-devin-mac.sh` 菜单 **23**（归档） |
...
| **设备 ID** | `~/.codeium/windsurf/installation_id` | Devin 设备标识（与登录关联） | ❌ 不能删 |
| **AI 索引** | `~/.codeium/windsurf/implicit/`、`code_tracker/` | 代码追踪索引，可重建 | ✅ 可清 |
| **VSCode 设置** | `~/Library/Application Support/Devin/User/settings.json` | 个人编辑器配置 | ❌ 不能删 |
| **快捷键** | `~/Library/Application Support/Devin/User/keybindings.json` | 自定义快捷键 | ❌ 不能删 |
| **全局状态** | `~/Library/Application Support/Devin/User/globalStorage/state.vscdb` | 工作区列表、最近文件 | 🟡 只能 VACUUM，不能删 |
...
| **机器 ID** | `~/Library/Application Support/Devin/machineid` | Electron 机器 ID | ❌ 不能删 |
| **登录 Cookies** | `.../Devin/Cookies`、`Cookies-journal` | 登录凭证 | ❌ 不能删 |
| **Local Storage** | `.../Devin/Local Storage/leveldb/` | 内嵌网页会话 | ❌ 不能删 |
| **WebStorage** | `.../Devin/WebStorage/` | 内嵌网页登录态 | ❌ 不能删 |
| **Electron Cache** | `.../Devin/Cache/`、`CachedData/`、`GPUCache/`、`Code Cache/`、`DawnWebGPUCache/`、`DawnGraphiteCache/` | Chromium 内核缓存 | ✅ 可清 |
| **Shared Dictionary** | `.../Devin/Shared Dictionary/` | 压缩字典缓存 | ✅ 可清 |
| **IndexedDB** | `.../Devin/IndexedDB/vscode-file_vscode-app_0.*` | **VSCode webview 的 UI 状态**（非登录！） | ✅ 可清 |
| **Service Worker** | `.../Devin/Service Worker/CacheStorage`、`ScriptCache` | Chromium SW 资源 | ✅ 可清 |
| **blob_storage** | `.../Devin/blob_storage/` | 临时 Blob 对象 | ✅ 可清 |
| **运行日志** | `.../Devin/logs/` | 文本日志 | ✅ 可清 |
| **崩溃报告** | `.../Devin/Crashpad/completed/`、`pending/` | 崩溃 dump | ✅ 可清 |
| **扩展旧包** | `.../Devin/CachedExtensionVSIXs/`、`CachedProfilesData/` | 扩展安装包残留 | ✅ 可清 |
| **终端快照** | `/tmp/devin-terminal-*.snapshot` | Cascade 终端快照 | ✅ 可清 |
...
### Q1：清理后 Devin 需要重新登录吗？
A：本工具的菜单 **2/3/18/20/23/24** 都不会清理登录相关目录，不会导致重新登录。只有菜单 **1**（会删对话）、**17**（重置 ID）、**18 第二阶段**（手动确认）才会影响登录。

### Q2：对话归档后怎么恢复？
A：归档目录里会自动生成 `restore.sh`，运行 `bash ~/.devin-conversations-archive-*/restore.sh` 即可一键恢复；或手动 `cp ~/.devin-conversations-archive-*/<file>.pb ~/.codeium/windsurf/cascade/`。
...
### Q4：Cascade 对话历史为什么会让 Devin 卡？
A：Devin 启动时会扫描 `~/.codeium/windsurf/cascade/` 目录，切换对话时要读取对应 `.pb`（protobuf 序列化）。单个对话文件可达 27MB+。当目录内有 50+ 个文件、累计 300MB+ 时，切换/启动延迟会很明显。归档到外部目录后，Devin 只需加载保留的近期对话，体感显著变快。

### Q5：为什么要保留 Devin 不自动清对话？
A：Devin 官方没有"清对话"按钮，但社区反馈显示 Cascade 的内存/IO 压力随对话历史线性增长（<https://boostdevspeed.com/blog/devin-ide-10gb-ram-memory-leak-fix>）。归档是当前唯一**既保留所有对话又缓解卡顿**的方案。
...
|:-:|:-|:-:|
| 每天 | 无需操作 | — |
| 每周一次 | `fix-devin-mac.sh` 菜单 **24**（卡顿诊断 + 一键优化） | 1 分钟 |
| 每月一次 | `fix-devin-mac.sh` 菜单 **20**（一键智能优化） | 2 分钟 |
...
- Devin 官方故障排除：<https://docs.devin.com/troubleshooting/devin-common-issues>
- VSCode state.vscdb VACUUM issue：<https://github.com/microsoft/vscode/issues/235684>
- Devin 内存泄漏分析：<https://boostdevspeed.com/blog/devin-ide-10gb-ram-memory-leak-fix>
