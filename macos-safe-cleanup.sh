...
if confirm "  是否清理用户应用缓存、日志以及照片ML分析数据？"; then
    ...
    if [ -d "$HOME/Library/Caches" ]; then
        PROTECTED_CACHES=(
            ...
            "com.exafunction.windsurf"          # Windsurf 基础缓存单独处理，避免误删登录态
...
if [ -d "$WS_DIR" ]; then
    ...
    if [ "$WS_CLEANABLE" -gt 0 ] 2>/dev/null; then
        ...
        if confirm "  是否一键清理 Windsurf 缓存、日志、AI索引并压缩数据库（不影响登录和历史对话，解决长对话卡顿）？"; then
            ...
            # 2. 清理 UI / 脚本缓存
            print_info "  已保留 Windsurf IndexedDB（可能含工作区状态，不作为纯缓存删除）"
            ...
            if [ -f "$STATE_BACKUP" ]; then
                safe_remove_rebuildable_path "$STATE_BACKUP" "Windsurf 旧 state.vscdb.backup"
            fi
            ...
            for ai_dir in "$HOME/.codeium/windsurf/implicit" "$HOME/.codeium/windsurf/code_tracker"; do
                if [ -d "$ai_dir" ]; then
                    safe_remove_rebuildable_path "$ai_dir" "Windsurf $(basename "$ai_dir") AI 索引缓存"
                fi
            done
