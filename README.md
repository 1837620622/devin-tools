...
跨平台 Devin IDE 故障排除工具包，重点解决启动卡顿、Shell 连接异常、
...
- 补充 Devin 常见问题、PowerShell 编码说明、MCP 排查入口和系统缓存清理。
- 保留对话历史、登录态、扩展配置、MCP 配置的风险边界说明，减少误删。

## 当前验证结论

结合本地目录勘察、Devin 官方排障文档、Electron 存储语义和 PowerShell
...
- macOS 系统清理：
  ...
  按风险等级清理系统缓存、开发工具缓存、Devin 运行时缓存和常见应用缓存。

## 仓库结构

| 文件 | 用途 |
| --- | --- |
| `fix-devin-mac.sh` | macOS 主修复脚本 |
| `fix-devin-linux.sh` | Linux 主修复脚本 |
| `fix-devin-win.ps1` | Windows 主修复脚本 |
| `fix-devin-win.bat` | Windows 批处理入口，便于直接启动 PowerShell 版本 |
| `macos-safe-cleanup.sh` | macOS 系统数据与开发工具缓存清理脚本 |
| `DEVIN-CLEANING-GUIDE.md` | Devin 长对话卡顿、清理边界与设备 ID 模式说明 |
| `README.md` | 中文说明 |
| `README.en.md` | 英文说明 |
...
`fix-devin-win.ps1` 使用 `UTF-8 with BOM`，用于兼容 Windows
...
[Devin 清理参考手册](./DEVIN-CLEANING-GUIDE.md)。
...
这些位置更接近 Electron 持久化站点或会话数据。清理后，Devin 内嵌网页、
认证状态或某些服务连接可能需要重新登录。
...
- 默认清理 Devin 的 `CachedData`、`Cache`、`GPUCache`、
...
- 对话历史：
  ...
  Windows 为 `%USERPROFILE%\.codeium\devin\cascade`。
- MCP 配置：
  macOS / Linux 为 `~/.codeium/windsurf/mcp_config.json`；
  Windows 为 `%USERPROFILE%\.codeium\devin\mcp_config.json`。
- macOS 运行时缓存：
...
- [Devin 官方排障文档](https://docs.devin.com/troubleshooting/devin-common-issues)
- [Devin Terminal 文档](https://docs.devin.com/devin/terminal)
- [Devin MCP 文档](https://docs.devin.com/devin/cascade/mcp)
